...
import { ErrorInfo, PageResult } from '../page-result'
...
export class PageProcessor {
  ...
  public async process(url: string, isInternal: boolean, state: CrawlerState): Promise<PageResult> {
  ...
  private getListeners(state: CrawlerState, key: keyof ScanListener, url: string) {
  ...
  private async handleOnPageLoad(listenerKey: keyof ScanListener, state: CrawlerState, pageResult: PageResult) {
  ...
  private async callHandler(handler: PageCheckReadyHandler, pageResult: PageResult, state: CrawlerState) {
...
function createIsIgnored(state: CrawlerState, pageResult: PageResult) {
  return (url: string) => {
